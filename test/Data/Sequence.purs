...
sequenceTests = do
  ...
  quickCheck $ \f z xs ->
    let
      types = Tuple (f :: Number -> Number -> Number) (z :: Number)
    in
      foldr f z (S.fromFoldable xs) == foldr f z (xs :: Array Number)

  quickCheck $ \f z xs ->
    let
      types = Tuple (f :: Number -> Number -> Number) (z :: Number)
    in
      foldl f z (S.fromFoldable xs) == foldl f z (xs :: Array Number)
  ...
  quickCheck $ \(ArbSeq xs) ->
    let
      xs' = S.cons 0 xs -- ensure xs' has at least one element
    in
      S.length xs' - 1 == S.length (S.drop 1 xs')
  ...
  quickCheck $ \idx (ArbSeq seq) ->
    ...
    in
      S.last (fst split) == S.index (idx' - 1) seq
  ...
  quickCheck $ \(ArbSeq seq) ->
    let
      f n = S.adjust identity n (seq :: S.Seq Number)
    in
      f (-1) == f (S.length seq)
  ...
  quickCheck $ \(ArbSeq seq) ->
    let
      f n = S.index n (seq :: S.Seq Number)
    in
      f (-1) == Nothing && f (S.length seq) == Nothing
  ...
  quickCheck $ \(ArbSeq seq) ->
    ...
    in
      S.inBounds lowerBound seq' && S.inBounds upperBound seq'
  ...
  quickCheck $ \(ArbSeq seq) n ->
    let
      result = S.length $ S.take n (seq :: S.Seq Number)
    in
      (0 <= result && result <= abs n) <?> err
        [ "n = " <> show n
        , "seq = " <> show seq
        , "result = " <> show result
        ]
  ...
  quickCheck $ \(ArbSeq seq) n ->
    let
      dropped = S.length (seq :: S.Seq Number) - S.length (S.drop n seq)
    in
      (0 <= dropped && dropped <= abs n) <?> err
        [ "n = " <> show n
        , "seq = " <> show seq
